...
You can visit the deployed app, at [https://gitname.github.io/deploy-react-here/](https://gitname.github.io/deploy-react-here/).
...
1. **Create an *empty* repository on GitHub.** (2 minutes)
    ...
    * For this tutorial, I'll create a repository named `deploy-react-here`.
...
2. **Create a new React app on your computer.** (5 minutes)
    ...
    ```sh
    $ create-react-app deploy-react-here
    ```
    
    * This is the app you will deploy to GitHub Pages in step 7.
    * I opted to give the app the same name as my GitHub repository (i.e. `deploy-react-here`). However, you can name them differently from one another (e.g. you can name your app `app-123` and your GitHub Repository `repo-456`).
    * This will create a new folder named `deploy-react-here` (or whatever you named your app) on your computer.

3. **Install the `gh-pages` package as a "dev-dependency" of the app.** (1 minute)

    ```
    $ cd deploy-react-here
...
4. **Add some properties to the app's `package.json` file.** (3 minutes)
    ...
    * At the top level, add a `homepage` property. Define its value to be the string `http://{username}.github.io/{repo-name}`, where `{username}` is your GitHub username, and `{repo-name}` is the name of the GitHub repository you created in step 1. Since my GitHub username is `gitname` and the name of my GitHub repository is `deploy-react-here`, I added the following property:
    
    ```js
    //...
    "homepage": "http://gitname.github.io/deploy-react-here"
    ```
    ...
    * **Shortcut:** Instead of adding those properties using a text editor; if I have `sed` installed on my system, I can add the properties by issuing the following shell commands:
    ...
    ```sh
    $ sed -i '5i\  "homepage": "http://gitname.github.io/deploy-react-here",' ./package.json
...
5. **Create a git repository in the app's folder.** (1 minute)
    ...
    ```
    $ git init
    Initialized empty Git repository in C:/path/to/deploy-react-here/.git/
    ```

6. **Add the GitHub repository as a "remote" in your local git repository.** (1 minute)

    ```
    $ git remote add origin https://github.com/gitname/deploy-react-here.git
    ```
...
7. **Generate a *production build* of your app, and deploy it to GitHub Pages.** (2 minutes)
    ...
    * In my case, my app is now accessible at: https://gitname.github.io/deploy-react-here/
